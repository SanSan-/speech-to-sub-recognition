...
и создания субтитров SRT, ASS или WebVTT. По умолчанию используется SRT. Основной режим
работает локально на машине пользователя. Если
...
видео или аудио -> ffprobe -> FFmpeg, FLAC mono 16 кГц
                ...
                -> необязательное выравнивание -> проверенные SRT, ASS или WebVTT
```
...
- выбор SRT, ASS или WebVTT в командном и веб-интерфейсах, по умолчанию SRT;
- общая разметка субтитров по готовым предложениям со смысловой балансировкой строк и диагностикой
...
формирования выбранного формата субтитров и публикации файлов не дублируются.
...
│  ├─ benchmark.py                 # метрики субтитров, памяти и видеопамяти
...
│  ├─ subtitles
│  │  ├─ builder.py                # слова и сегменты -> общие реплики
│  │  ├─ formats.py                # вывод SRT, ASS и WebVTT
...
├─ .github
│  └─ workflows/python-release-build.yml # сборка релизных артефактов
├─ docs
...
и словные временные метки, необходимые общему сборщику субтитров. Локальный режим остаётся основным.
...
Для `<media-directory>\lecture.mp4`, языка `en` и SRT по умолчанию создаются прежние артефакты:

```text
<media-directory>\lecture.en.srt
<media-directory>\lecture.en.asr.json
<media-directory>\lecture.en.asr.flac        # только при ASR_KEEP_AUDIO=1
```

- `.srt` — проверенные субтитры SRT в UTF-8 без BOM;
- `.asr.json` — контрольный отпечаток источника, выбранный поток, движок и параметры,
  ...
  границ субтитров и число узких нормализаций растянутых якорей `timing_anomaly_adjustments`;
- `.asr.flac` — необязательный нормализованный звук при `ASR_KEEP_AUDIO=1`.

При выборе ASS или WebVTT создаются раздельные форматные артефакты:

```text
<media-directory>\lecture.en.ass
<media-directory>\lecture.en.ass.asr.json
<media-directory>\lecture.en.ass.asr.flac    # только при ASR_KEEP_AUDIO=1
<media-directory>\lecture.en.vtt
<media-directory>\lecture.en.vtt.asr.json
<media-directory>\lecture.en.vtt.asr.flac    # только при ASR_KEEP_AUDIO=1
```

Сопроводительный JSON и результат содержат `output_format` и общий путь `subtitle_output`.
Совместимое поле `srt_output` равно этому пути только для SRT и равно `null` для ASS и WebVTT.
Необязательный нормализованный звук входит в тот же
форматно изолированный набор: SRT сохраняет прежнее имя, ASS и WebVTT включают формат в имя.

Субтитры выбранного формата, сопроводительный JSON и сохраняемый FLAC сначала готовятся во
временных соседних файлах в UTF-8 без BOM.
Готовый набор публикуется атомарно; при частичном сбое восстанавливается предыдущая согласованная
версия. Пустые или не прошедшие профильную проверку субтитры не становятся готовым результатом.
Выбор одного формата не удаляет и не заменяет готовые артефакты другого формата.
...
## Правила формирования субтитров
...
- сохраняет положительные непересекающиеся интервалы и после округления допускает выход за
  длительность аудио не более чем на неизбежную часть кванта выбранного формата;
- использует минимальный положительный интервал в один квант выбранного формата;
...
Одна и та же последовательность реплик передаётся профильному формирователю:

- SRT нумерует блоки с единицы и использует время `HH:MM:SS,mmm` с точностью 1 мс;
- WebVTT начинается строкой `WEBVTT`, использует время `HH:MM:SS.mmm` с точностью 1 мс и
  защищает распознанный текст от интерпретации как управляющей разметки;
- ASS содержит разделы `[Script Info]`, `[V4+ Styles]` и `[Events]`, закреплённый стиль
  `Default`, строки `Dialogue`, время `H:MM:SS.cc` с точностью 10 мс и перенос `\\N`;
  распознанный текст не интерпретируется как управляющая последовательность ASS.
...
описывает тяжёлое распознавание и выравнивание, а `layout_settings` — лёгкую раскладку и
выходной формат. Полностью готовый кеш требует совпадения обоих отпечатков и валидных
субтитров выбранного формата.

- совпавшие валидные результаты получают состояние `cached` без загрузки модели;
- при совпадении `recognition_settings` отсутствующий выбранный формат формируется из
  сохранённого `Transcript` без повторного запуска модели; при `keep_audio=false` FFmpeg также
  не запускается, а при сохранении FLAC выполняется только нормализация для отсутствующего
  форматного аудиофайла; источником может быть совместимый сопроводительный JSON SRT, ASS или
  WebVTT;
- существующие целевые субтитры без подтверждающего JSON не перезаписываются и получают
  состояние `skipped`;
- существующие целевые субтитры при несовпавшей разметке также не заменяются без `--force`;
- `--force` обходит кеш готовых субтитров и сохранённого распознавания, повторно выполняет
  нормализацию аудио, движок распознавания и выбранное выравнивание, после чего атомарно
  заменяет вычисленные целевые субтитры, JSON и сохраняемый FLAC выбранного запуска, не
  затрагивая артефакты других форматов;
...
Параметры длинных записей входят в тяжёлый отпечаток, а параметры разметки и `output_format` —
в лёгкий. Если выбранный формат отсутствует, совпавшее распознавание из совместимого
сопроводительного файла SRT, ASS или WebVTT может быть использовано для его формирования без
`--force`.
...
модель, язык, аудиопоток, устройство, модуль выравнивания, `--output-format srt|ass|vtt`,
ограничения раскладки, управление
...
Без `--output-format` создаётся SRT. Например, `--output-format ass` формирует ASS, а
`--output-format vtt` — WebVTT.
...
длительность медиа, аудиопотоки, выбранную дорожку, выбранный формат субтитров, текущую стадию,
ход выполнения, общий путь результата и ошибку. Во время обработки свежая длительность появляется сразу после рабочего `ffprobe` и
...
В основных настройках доступен выбор SRT, ASS или WebVTT; по умолчанию выбран SRT. Значение
`output_format` сохраняется в `localStorage`, а прежний или повреждённый снимок без допустимого
значения восстанавливается как SRT. Изменение настройки сохраняет новое значение, но не запускает скрытую повторную проверку всей
...
- редактор субтитров и встраивание SRT, ASS или WebVTT обратно в контейнер не реализованы;
...
Публикация GitHub Release запускает
`.github/workflows/python-release-build.yml`: на Python 3.14 собираются `sdist` и wheel и
сохраняются в артефакте `release-dists` соответствующего запуска GitHub Actions. Автоматическая
публикация во внешний реестр этим сценарием не выполняется.

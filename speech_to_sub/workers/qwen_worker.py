...
def handle_request(
    ...
    try:
        if command == "ping":
            ...
            result = runtime.transcribe(payload, progress_callback)
        elif command == "unload":
            ...
            raise ValueError(f"Неизвестная команда Qwen worker-а: {command or '?'}")
...
def _resolve_device(torch: Any, requested_device: str) -> tuple[str, str, Any]:
    ...
    if requested not in {"auto", "cuda", "cpu"}:
        raise ValueError("Устройство Qwen3-ASR должно быть auto, cuda или cpu.")
